"""Validated runtime configuration for the productivity assistant."""
...
def _legacy_prototype_enabled() -> bool:
    return os.getenv("PROTOTYPE_MODE", "").strip().lower() in {"1", "true", "yes"}
...
class Settings:
    ...
    def from_env(cls) -> "Settings":
        configured_mode = os.getenv("APP_MODE", "").strip().lower()
        app_mode = configured_mode or (
            "prototype" if _legacy_prototype_enabled() else "full"
        )
